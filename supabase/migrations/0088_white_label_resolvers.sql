-- ═══════════════════════════════════════════════════════════════════════════
-- 0088 — expose white_label to the web chat resolvers
--
-- Embedded on a customer's own site, the widget should read as THEIR brand: the
-- store logo + display name, not Rani's face + "Ask Rani". The header now leads
-- with the store identity; a small "Powered by Ask Rani" attribution stays by
-- default and is removed when the owner turns on white-label (a paid lever).
--
-- The flag is an agent_config row ('white_label' = 'true', enum added in 0087).
-- It rides in the token payload both public surfaces already read, so both
-- resolvers must expose it. Definitions are 0057 (validate_store_token) and
-- 0086 (resolve_store_by_key) verbatim + the one new field.
-- ═══════════════════════════════════════════════════════════════════════════
...
as $$
  select json_build_object(
    ...
    'white_label', coalesce((
      select lower(btrim(c.value)) = 'true' from public.agent_config c
      where c.store_id = s.id and c.key = 'white_label' limit 1
    ), false)
  )
