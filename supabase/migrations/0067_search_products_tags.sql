-- 0067 — search_products returns allergens + dietary, so the assistant can answer
-- "is the biryani nut-free?" / "what's vegan?" truthfully from the item's own tags.
-- Return type changes, so drop + recreate (0045 body + two columns).
...
as $$
  ...
  select p.id, p.sku, p.name, p.brand, p.size, p.unit, p.price, p.currency,
         p.in_stock, p.category, p.description, p.image_url, p.allergens, p.dietary, f.score
  from fused f
