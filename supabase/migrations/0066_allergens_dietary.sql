-- 0066 — Allergen & dietary data on products (compliance + diner filtering)
--
-- Two structured, filterable arrays per product:
--   allergens  — "contains" declarations from the EU-14 / US-9 canonical set
--                (gluten, milk, eggs, fish, crustaceans, tree_nuts, peanuts, soy,
--                 sesame, celery, mustard, sulphites, lupin, molluscs)
--   dietary    — positive claims a diner filters on
--                (vegetarian, vegan, gluten_free, halal, kosher, dairy_free, nut_free)
-- Both default to '{}' so contains/overlap operators and facet counts are clean.

alter table public.products add column if not exists allergens text[] not null default '{}';
alter table public.products add column if not exists dietary   text[] not null default '{}';

create index if not exists idx_products_allergens on public.products using gin (allergens);
create index if not exists idx_products_dietary   on public.products using gin (dietary);

comment on column public.products.allergens is
  'Allergens the item CONTAINS (canonical ids). Used for "avoid" filtering + allergen answers.';
comment on column public.products.dietary is
  'Positive dietary claims (vegetarian/vegan/gluten_free/halal/…). Used for include filtering.';

-- ── browse_products v2: add dietary (include-all) + allergen (exclude-any) ──────
-- Adding parameters changes the signature, so drop the old overload first.
drop function if exists public.browse_products(
  uuid, text, extensions.vector, text[], text[], numeric, numeric, boolean, text[], int, int, boolean
);
...
create or replace function public.browse_products(
  ...
  p_dietary           text[] default null,   -- item must have ALL of these
  p_exclude_allergens text[] default null     -- item must contain NONE of these
)
returns json
language sql
security definer
...
as $$
  with filtered as (
    select p.id, p.sku, p.name, p.brand, p.size, p.unit, p.price, p.currency,
           p.category, p.image_url, p.in_stock, p.embedding, p.allergens, p.dietary
  ...
  select json_build_object(
    ...
    'items', coalesce((
      select json_agg(json_build_object(
        ...
        'allergens', r.allergens, 'dietary', r.dietary
    ...
    'facets', json_build_object(
      ...
      'dietary', coalesce((
        select json_agg(d) from (
          ...
          from base, unnest(base.dietary) as tag
      ...
      'allergens', coalesce((
        select json_agg(a) from (
          select tag as value, count(*)::int as count
          from base, unnest(base.allergens) as tag
          group by tag order by count(*) desc, tag asc
        ) a
      ), '[]'::json),
